...
#Import the rds file
import_martina  <- readRDS("ISVARIC_dash_db_20201117_preprocess_old.rds")
ls(import_martina)
...
#Resp
data_plot_vs_resp <- select(import_martina, c(starts_with("slider"),vs_resp)) %>%
...
#hr
data_plot_vs_hr <- select(import_martina, c(starts_with("slider"),vs_hr)) %>%
  pivot_longer(starts_with("vs"), names_to = "symptom", values_to = "value") %>%
  filter(!is.na(value)) %>%
  filter(!is.na(slider_agegp10)) %>%
  filter(value<(quantile(value, 0.75)+(1.5*IQR(value)))) %>%
...
#vs_temp
data_plot_vs_temp <- select(import_martina, c(starts_with("slider"),vs_temp)) %>%
  pivot_longer(starts_with("vs"), names_to = "symptom", values_to = "value") %>%
  filter(!is.na(value)) %>%
  filter(!is.na(slider_agegp10)) %>%
  filter(value<(quantile(value, 0.75)+(1.5*IQR(value)))) %>%
...
#vs_sysbp
data_plot_vs_sysbp <- select(import_martina, c(starts_with("slider"),vs_sysbp)) %>%
  pivot_longer(starts_with("vs"), names_to = "symptom", values_to = "value") %>%
  filter(!is.na(value)) %>%
  filter(!is.na(slider_agegp10)) %>%
  filter(value<(quantile(value, 0.75)+(1.5*IQR(value)))) %>%
...
#vs_oxysat
data_plot_vs_oxysat <- select(import_martina, c(starts_with("slider"),vs_oxysat)) %>%
  pivot_longer(starts_with("vs"), names_to = "symptom", values_to = "value") %>%
  filter(!is.na(value))  %>%
  filter(!is.na(slider_agegp10)) %>%
  filter(value<(quantile(value, 0.75)+(1.5*IQR(value)))) %>%
...
p_resp <- ggplot(data = data_plot_vs_resp, aes(x=slider_agegp10, y=value)) + 
  geom_boxplot(aes(fill=slider_agegp10)) +xlab("Age groups") + ylab("signs") + 
