...
data_pre <- read_csv("ISVARIC_dash_db_20201117_preprocess.csv")
...
df=data.preprocessing(data_pre)
...
###################11.17
df <- data %>% 
  mutate(admission.to.icu=difftime(ymd(icu_in), ymd(date_admit), units="days")) 
outcome.admission.date.input <- outcome.admission.date.prep(df)
save(outcome.admission.date.input, file="outcome_admission_date_input.rda")
